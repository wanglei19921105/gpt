	...
	<view>
		...
		<view v-if="data.friends.length>0">
			<view class="cu-bar padding-left light" :class="'bg-'+actcolor">
				<text class="text-lg">请选择您要分享的好友</text>
			</view>
...
<script>
	...
	export default {
		...
		onLoad(options) {
			if(typeof(options.cid)=='undefined' || !options.cid) return core.toast('无效的会话标识','back');
			this.postdata.cid = options.cid;
			if(typeof(options.type)!='undefined' && options.type!='') this.postdata.type = options.type;
			let that = this;
			this.initData('friend/card',{},function(res){
		...
		methods:{
			...
			doShareCard(uid){
				...
				return core.confirm('确定要分享该好友名片到聊天中吗？','分享名片',function(){
					core.post('friend/card',function(res){
