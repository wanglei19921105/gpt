<template>
	<view class="page" :class="getThemeClass">
			...
			<view class="head_cent">
				<view class="head_upperpart">
					<view class="price_number">
						...
						<text class="size-22">≈￥{{(product_cast.up_price*exchange_rate).toFixed(2)}}</text>
					</view>
				</view>
		...
		<!-- 藏品详情 -->
		<!-- <explain text="藏品详情" :cont="product_cast.desc"></explain> -->
...
<view class="gap-200"></view>
		...
		<view class="buy card-bg border-color">
			...
			<template v-if="new_type === 0">
				<view class="buy-countdown" v-if="product_cast.diff > 0 && !(product_cast.inventory == 0 || product_cast.show_type === 3)">
					<template v-if="product_cast.diff > 0 && product_cast.shou_type === 0">
						...
						<u-count-down 
							...
							:bg-color="getBgColor"
							@end="produictDet(id)"
							:timestamp="product_cast.diff + 3"
							font-size="24"
							separator="zh"
						></u-count-down>
					</template>
					<template v-if="product_cast.diff > 0 && product_cast.shou_type === 1">
			...
			<template v-if="new_type == 2">
				<view class="buy-countdown" v-if="first_diff > 0">
					<template v-if="user_lucky_status === 0">
						<text class="">开售倒计时：</text>
						<u-count-down 
							:separator-color="getMainColor"
							:color="getMainColor" 
					...
					<template v-if="user_lucky_status === 1">
						<text class="">开奖倒计时：</text>
						<u-count-down 
							:separator-color="getMainColor"
							:color="getMainColor" 
							:bg-color="getCardColor"
					...
					<template v-if="user_lucky_status === 3">
						<text class="">商品上架倒计时：</text>
						<u-count-down 
							:separator-color="getMainColor"
							:color="getMainColor" 
							:bg-color="getCardColor"
			...
			<view class="buy-content">
				...
				<view class="" v-if="new_type == 0">
					...
					<view class="" v-else>
						<botBtn
							v-if="product_cast.diff > 0 && product_cast.shou_type === 0"
							padding="20rpx 60rpx"
							radius="50rpx"
							disabled
						>
						...
						<botBtn
							@click="buy"
							v-if="(product_cast.diff > 0 && product_cast.shou_type === 1) || (product_cast.diff == 0 && product_cast.shou_type === 0)"
							padding="20rpx 60rpx"
							radius="50rpx"
						>立即购买</botBtn>
						
						<botBtn
							v-if="product_cast.diff === 0 && product_cast.shou_type === 1"
							padding="20rpx 60rpx"
							radius="50rpx"
							disabled
						>发售结束</botBtn>
				...
				<view class="" v-if="new_type == 2">
					<template v-if="user_lucky_status == 0 && first_diff == 0 && product_cast.inventory == 0">
						<botBtn padding="20rpx 60rpx" radius="50rpx" disabled>已售罄</botBtn>
					</template>
					<template v-else>
						<botBtn padding="20rpx 60rpx" radius="50rpx" disabled v-if="user_lucky_status == 0 && first_diff > 0">等待开售</botBtn>
						<botBtn padding="20rpx 60rpx" radius="50rpx" @click="chouqian" v-if="user_lucky_status == 0 && first_diff == 0">抽签</botBtn>
						<botBtn padding="20rpx 60rpx" radius="50rpx" disabled v-if="user_lucky_status == 1">已抽签，等待结果</botBtn>
						<botBtn padding="20rpx 60rpx" radius="50rpx" disabled v-if="user_lucky_status == 2">很遗憾，未中签</botBtn>
						<botBtn padding="20rpx 60rpx" radius="50rpx" disabled v-if="user_lucky_status == 3">已中签，等待上架购买</botBtn>
						<botBtn padding="20rpx 60rpx" radius="50rpx" @click="buy(1)" v-if="user_lucky_status == 4">立即购买</botBtn>
						<botBtn padding="20rpx 60rpx" radius="50rpx" disabled v-if="user_lucky_status == 5">停止购买</botBtn>
					</template>
				</view>
				...
				<view class="" v-if="new_type == 3">
					...
					>已售罄</botBtn>
					<template v-else>
						<botBtn padding="20rpx 60rpx" radius="50rpx" @click="buy(1)" v-if="first_diff > 0 && user_is_first == 1">优先购</botBtn>
						<botBtn padding="20rpx 60rpx" radius="50rpx" 
							v-if="first_diff > 0 && user_is_first == 0"
							disabled
						>
							<text>等待开售</text>
						</botBtn>
						<botBtn padding="20rpx 60rpx" radius="50rpx" v-if="first_diff > 0 && user_is_first == 2" @click="buy(1)">
							<text>立即购买</text>
						</botBtn>
						<botBtn padding="20rpx 60rpx" radius="50rpx" v-if="user_is_first == 3" @click="buy(1)">
							<text>立即购买</text>
						</botBtn>
						<botBtn padding="20rpx 60rpx" radius="50rpx" v-if="first_diff == 0 && user_is_first == 2" disabled>发售结束</botBtn>
					</template>
				</view>
			</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			chouqian(){
				this.$u.api.lucky_join({cast_id:this.id}).then(res=>{
					if(res.code==200){
						uni.showToast({
							title:"已抽签，请等待开签",
							icon:"none"
			...
			// 立即购买按钮	
			buy(data) {
				if (this.userId == this.product_cast.author_user_id) {
				...
				let type = data||0
				uni.navigateTo({
					url: '../casting-buy/casting-buy-one?id=' + this.product_cast.id+'&type='+type
				})
