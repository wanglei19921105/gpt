		...
		<!-- ,{backgroundImage: `url(/static/images/my/${getThemeName}/Homecard.png)`} /static/images/index/${defaultThemeName}/goods_item_bg.png-->
			...
			<template >
				<view class="goods-image-box">
					...
					></image>
					<!-- 喜欢/取消喜欢 -->
					<view class="like" v-if="listType === 'circulation'"><!--/static/images/heart1.png  /static/images/heart1.png  @click.stop="likeShow(goods.id,goods.status)"-->
						<image v-if="curScreenStyle"
							:src="goods.status == 1?'/static/images/darkLike1.png':''"
							mode="widthFix">
							<image v-if="!curScreenStyle"
								:src="goods.status == 1?'/static/images/heart2.png':''"
								mode="widthFix">
							</image>
					</view>
					<view class="tuishi" v-if="goods.is_delisted == 1">
						<image src="../../static/images/delisting.png" mode="widthFix"></image>
					</view>
				...
				<view class="goods-content"  :style="doubleLine?'margin-top: -15rpx;':''">
					...
					<view class="goods-info-box u-m-t-20 justify-start" v-if="listType !== 'circulation'">
							...
							<view :class="doubleLine? 'u-flex-col' : 'u-flex align-center'">
								<view class="u-flex align-center justify-center">
									<view class="goods-num-box card-tag-bg">
										<text class="goods-num">发行</text>
										<text class="goods-num-unit">{{ goods.total_issue }}份</text>
									</view>
									<view class="goods-num-box card-tag-bg u-m-l-20" v-if="goods.inventory !== 0">
										<text class="goods-num">剩余</text>
										<text class="goods-num-unit">{{ goods.inventory }}份</text>
									</view>
						...
						<template v-if="listType === 'blind'">
							<view class="goods-num-box card-tag-bg" v-if="goods.num !== 0">
								<text class="goods-num">限量</text>
								<text class="goods-num-unit">{{ goods.num }}份</text>
							</view>
					...
					<view class="goods-bottom-box" :class="doubleLine? 'goods-bottom-box-s' :'' " style="margin-top: 10rpx;">
						...
						<view class="goods-price-box">
							...
							<text class="">≈￥{{(goods.price*exchange_rate).toFixed(2)}}</text>
						</view>
					</view>
				</view>
			</template>
