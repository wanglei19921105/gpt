	...
	<view class="page" :class="getThemeClass">
		...
		<view class="card card-bg">
			<view class="cover_img">
				<image :src="itemData.cover" style="border-radius: 10rpx;" mode="widthFix"></image>
			</view>
		...
		<!-- 请输入转出的用户名 -->
		<input class="myinput card-bg" type="text" v-model="params.other_info" :placeholder="i18n.derivatives1"
			placeholder-class="input_pla text-placeholder" />
		<!-- 安全密码 -->
		<view class="title">{{i18n.Copyright3}}</view>
		<!-- 输入安全密码 -->
		<input class="myinput card-bg" type="text" v-model="params.security" :placeholder="i18n.Copyright4"
			placeholder-class="input_pla text-placeholder" />
		<view class="hint color-red">
...
<script>
	export default {
		...
		onLoad(option) {
			...
			this.params.product_pre_id = option.id
			this.loadData()
		},
		methods: {
			loadData(){
				this.$u.api.my_wallet_object_detail({id: this.params.product_pre_id}).then(res => {
					// console.log(res);
