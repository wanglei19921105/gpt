	...
	<view class="page" :class="getThemeClass">
		...
		<view class="compound-card card-bg" v-for="(item,index) in list" :key="index" @click="godetail(item.id)">
			...
			<view class="compound-card-content">
				...
				<view class="compound-time">
					<view class="size-20 flex justify-between">
						<view class="color-9 u-p-b-10">常规合成时间:</view>
...
<script>
	export default{
		...
		methods:{
			...
			godetail(id){
				uni.navigateTo({
					url: "/pages/market/compound/compound-launch-detail?id=" + id +"&&type="+0
				})
			}
		}
	}
