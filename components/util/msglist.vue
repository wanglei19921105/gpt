<template>
	<view class="cu-item dialogitem" :class="[message.uid==Number(userId)?'self':'',message.datetime==''?'notime':'']">
		<view class="cu-avatar radius 1" @longpress.stop="doAt()" @tap="naviTo('member/index',{uid:message.uid,fg:message.gid,fp:chatevel?'dialog':'favor'})" v-if="message.uid!=Number(userId)" :style="'background-image:url('+message.avatar+');'"></view>
		<view class="nickname text-grey" v-if="message.uid!=Number(userId) && (message.gid>0 || !chatevel)">{{message.nickname}}<text v-if="message.subtitle!=''" class="cu-tag sm bg-green round margin-left-xs">{{message.subtitle}}</text></view>
		<view :class="[(message.special.type=='normal'?'highmode'+message.highmode:'special-'+message.special.type),(message.special.type=='shareinfo'?'special-main':'main'), (message.uid==Number(userId)?'justify-end':'')]" class="flex-wrap" :id="'msgitem'+message.id" @longpress.stop="doLongPress" :data-index="msgindex">
			...
			<block v-if="message.special.type=='normal'">
				...
				<view class="content shadow" :style="message.uid==Number(userId)?'background-color:'+theme.chatbg:''" v-else>
					...
					<view class="chat-audio" :class="audiostyle[message.special.value]" @tap.stop="doPlayAudio()" v-if="message.highmode==5">
						<text class="cuIcon-sound" v-if="message.uid!=Number(userId)"></text>
						<text class="chat-audio-time">{{message.yuyintime}}</text>
						<text class="cuIcon-sound self" v-if="message.uid==Number(userId)"></text>
					</view>
				</view>
			</block>
				...
				<view class="content chat-transfer" @click="naviTo('credit/index')">
					<view class="flex align-center">
						...
						<view class="flex-fives padding-left-sm">
							...
							<view class="margin-top-xs">{{message.special.value.remark || (message.uid==Number(userId)?'转账给好友':'收到好友转账')}}</view>
						</view>
					</view>
			...
			<block v-if="message.special.type=='shareinfo'">
				<view class="cu-card case special-content share-large" :class="message.special.value.extra" v-if="message.special.value.style=='large'">
					<view class="cu-item shadow" @click="naviTo(message.special.value.page)">
						<view class="image">
							<image :src="message.special.value.cover" mode="widthFix"></image>
							<view class="cu-bar bg-shadeBottom"> <text class="text-cut">{{ message.special.value.title }}</text></view>
						</view>
					</view>
				</view>
				<view class="cu-list menu-avatar special-content share-link" :class="message.special.value.extra" v-else>
					<view class="cu-item arrow" @click="naviTo(message.special.value.page)">
						<view class="cu-avatar radius lg 2" :style="'background-image:url('+message.special.value.cover+');'"></view>
		...
		<view class="cu-avatar radius 3" @click="naviTo('member/index',{uid:message.uid})" v-if="message.uid==Number(userId)" :style="'background-image:url('+message.avatar+');'"></view>
		<view class="date">
			<block v-if="message.uid==Number(userId) && showRead">
...
<script>
	...
	export default {
		...
		methods:{
			...
			viewImage(pic){
				let shareUrl = core.page('dialog/share', {mid:this.message.id});
				return core.viewImage(pic, this.chatimages, shareUrl, "转发给好友");
			},
			posterReload(){
...
<style>
	...
	.special-content .cu-item{z-index: 101;}
	...
	.self .chat-audio.unread::after{display: none;}
	...
	.video-player .text-cut::before{content: "\e7c7"; font-family: cuIcon; font-style: normal; font-size: 48upx; color: #E18F39; vertical-align: middle; margin-right: 10upx;}
	...
	.dialogitem>.main .content.chat-transfer{display: block; padding: 40upx 30upx; padding-bottom: 80upx; min-width:380upx; border-radius: 8upx;background-color: #f99f3e;color: #FFFFFF !important;}
	...
	.share-link .cu-item{padding: 16upx; padding-right: 40upx !important;}
	.share-link .cu-item>.cu-avatar{left: 20upx; background-color: unset;}
	.share-link .cu-item>.content{left: 132upx; width: calc(100% - 176upx) !important;}
	.share-link .cu-item.arrow::before{right: 20upx;}
	.share-large .cu-item{margin: 0; padding: 0;}
	.share-large .cu-bar{min-height: 68upx;}
	.share-large .image{max-height: 450upx;}
