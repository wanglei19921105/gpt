<template>
	<view class="page" :class="getThemeClass">
		...
		<view class="head card-bg">
			<!-- 图片 -->
			<image :src="item.cover" style="border-radius: 30rpx 30rpx 0rpx 0rpx;" mode="widthFix"></image>
			<!-- NFT铸品名称 -->
		...
		<view class="input_box card-bg">
			<!-- 手续费 -->
			<view class="input_flex" style="margin-bottom: 32rpx;">
		...
		<!-- 售出 -->
		<botBtnp style="margin: 78rpx 0 40rpx;" :btnText="i18n.listed6" @click="sold"></botBtnp>
		<u-toast ref="uToast" />
...
<script>
	export default {
		...
		methods: {
			loadData(){
				this.$u.api.cast_detail({id: this.params.order_id}).then(res => {
					// console.log(res);
					this.item = res.data.product_cast
				})
			},
			// 密码弹窗点击立即支付后
			padpay(i) {
