<template>
	<view class="progress-bar" :style="'min-height:' + height + 'rpx'">
		...
		<view v-if="isCircular" class="progress-bar-circular"
			...
			<view v-if="showText" class="progress-bar-text">
        <text v-if="showTextValue" class="Block"  :style="{ color: textColor, fontSize: textSize + 'rpx' }">{{ showTextValue }}</text>
        <text v-if="showTextValue1" class="Block TextCenter MarginT_4rpx"  :style="{ color: textColor, fontSize: textSize1 + 'rpx' }">{{ showTextValue1 }}</text>
			</view>
		</view>
	</view>
</template>
