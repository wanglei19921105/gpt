<template>
	<view class="page" :class="getThemeClass">
		...
		<!-- 支付方式 -->
		<payWay @payway="payType" :onlyAward="true"></payWay>
		<!-- 立即支付 -->
		<botBtnp class="bot_btn" :btnText="i18n.castingbuy9" :go="go" @click.native="payStatus.show=true"></botBtnp>
...
export default {
	...
	methods: {
		...
		payBtn() {
			...
			this.$u.api.point_order_add({
				...
				if (res.code == 200) {
					...
					if(this.payway==1){
						this.$u.api.pre_pay_wechat(params).then(res => {
							if (res.code == 200) {
								// #ifdef APP-PLUS
								...
								// #endif
								// #ifdef H5
								this.$pay.wechatPay(res.data.result.url)
								// #endif
							}else{
								this.gowaitpay()
							}
						...
						this.$u.api.pre_pay_ali(params).then(res => {
							if (res.code == 200) {
								// #ifdef APP-PLUS
								...
								// #endif
								// #ifdef H5
								this.$pay.alipayPay(res.data.result)
								// #endif
							}else{
								this.gowaitpay()
							}
		...
		awardPay(){
			...
			this.$u.api.point_order_add(data).then(res => {
				if (res.code == 200) {
					this.paySuccess()
				}else{
					// this.gowaitpay()
