	...
	<view>
		...
		<view class="bg-white">
			<view class="cu-list menu-avatar padding-top-xs">
				<view class="cu-item padding-bottom" style="height: 200upx;">
					<view class="cu-avatar radius xxl" @click="doPrevImg(data.member.avatar)" style="top:20upx;border-radius:50%" :style="'background-image:url('+data.member.avatar+');'"></view>
					<view class="content" style="top: 0;left: 20px!important;">
						<view class="text-xxxl text-bold text-black" style="height: 100upx;">
                            <text style="font-size: 18px">{{data.friend.remark || data.member.nickname}}</text>
						    <view class="level">{{ data.member.levelName }}</view>
                        </view>
                        <view class="text-grey" style="width: 60%;height: 28upx">
                            <progress :percent="data.member.myVipProgress" stroke-width="6" activeColor="#4adce0" border-radius="4" />
                        </view>
                        <view class="text-grey">
                            <view>{{ data.member.vipName }}</view>
                        </view>
					</view>
				</view>
			</view>
...
<!--                    <view class="title" style="border: none">今日佣金</view>-->
<!--                </view>-->
<!--            </view>-->
            <view class="team">
                <view class="title">
                    <view>团队规模:{{ data.teamTotal }}人</view>
                    <view style="font-size: 12px;color:#6f6e6e" @click="$u.route('/pages/my/myTeam/myTeam')">查看我的团队 ></view>
                </view>
                <view class="members">
                    <view class="member-info" v-for="(item, index) in data.team" :key="index">
                        <image
                            class="avatar"
                            :src="item.avatar"
                            mode="aspectFill"
            ...
            <view class="extend">
                <view class="cont">
                    <view>
                        <uni-icons type="shop" size="20" style="margin-right: 30upx" color="#fff"/>
                        推广收益明细
                    </view>
                    <view> > </view>
                </view>
                <view class="cont">
                    <view>
                        <uni-icons type="bars" size="20" style="margin-right: 30upx" color="#fff"/>
                        项目收益明细
                    </view>
                    <view> > </view>
                </view>
            </view>
			<view class="cu-list menu" style="margin: 20px;border-radius: 6px">
		...
		<view style="padding: 0 20px;border-radius: 6px" class="cu-list menu margin-top-sm text-darkblue text-center" v-if="userId!=data.member.uid">
			<block v-if="!groupprotective">
...
<script>
    ...
    import LimePainterView
        from "../../uni_modules/uni_modules/lime-painter/components/l-painter-view/l-painter-view.vue";
    import UniIcons from "../../uni_modules/uni-icons/components/uni-icons/uni-icons.vue";
	export default {
		computed: mapState(['userId']),
		components: {UniIcons, LimePainterView, share},
		data() {
			return {
				loaded:false,
				uid:0,
...
<style>
    ...
    .earnings .earnings-content .title{
        font-size: 12px;
        height: 29px;
        line-height: 34px;
        text-align: center;
