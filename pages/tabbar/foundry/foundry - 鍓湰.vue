<template>
	<view class="page" :class="getThemeClass">
		...
		<view class="upload_box card-bg">
			<view class="upload_files card-bg">
				<view class="hint">
					...
					<!-- JPG,PNG,GIF,MP3,MP4,{{ i18n.casting3 }}100MB -->
		...
		<!-- 版税 -->
		<view class="royalty">
			<view class="title no_margin">{{ i18n.casting6 }}</view>
			<view class="step">
		...
		<!-- 副本 -->
		<view class="title">{{ i18n.casting7 }}</view>
		<!-- 输入商品副本数量 -->
		...
		<u-input class="sort_select card-bg" height="84" v-model="sortValue" type="select" @click="show = true" />
		...
		<!-- <u-modal
			class="my-modal"
			v-model="showGoArtist" 
			content="暂未通过艺术家认证，认证后方可铸造，请前往认证！" 
			title="提示"
			:confirm-color="getMainColor" 
			@confirm="true || $u.route('/pages/my/creator/creator')"
...
.page-black {
	...
	.night {
		/deep/.botBtn {
			background: linear-gradient(0deg, #f7924c, #eeb32e);
		}
	}
}
