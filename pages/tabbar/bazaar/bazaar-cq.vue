<template>
	<view class="page" :class="getThemeClass">
		...
		<my-goods-list :listData="listData" :listStatus="listStatus" :doubleLine="isDouble" listType="draw" @click="clickGoods" />
		...
		<view class="featured" :class="getThemeClass" v-if="false">
					<view class="featured_item card-bg" v-for="(item, index) in listData" :key="index" @click="goDetails(item.id)">
						<view class="plan">
							...
							<view class="presale" v-if="item.products_status == 1">
								<count v-if="item.diff > 1" :end="item.diff"></count>
								<!-- 后开始抢购 -->
								{{ i18n.market22 }}
							</view>
							<view class="presale" v-if="item.products_status == 2">
								<!-- 拍卖已结束 -->
								<text v-if="item.diff < 1">{{ i18n.market23 }}</text>
								<!-- 拍卖结束于 -->
								<text v-if="item.diff > 1">{{ i18n.market24 }}：</text>
								<count v-if="item.diff > 1" :end="item.diff"></count>
							</view>
							<view class="type">抽</view>
						</view>
				...
				<view class="featured" :class="getThemeClass" v-if="false">
					<view class="featured_item flex" style="width: 690rpx;" v-for="(item, index) in listData" :key="index" @click="goDetails(item.id)">
						<view class="">
							<image :src="item.cover || ''" mode="aspectFit" style="width: 260rpx;height: 260rpx;"></image>
							<view class="presale" v-if="item.products_status == 1">
								<count v-if="item.diff > 1" :end="item.diff"></count>
								<!-- 后开始抢购 -->
								{{ i18n.market22 }}
						...
						<view class="featured_content flex-1 flex flex-direction justify-around">
							<view class="flex justify-between align-center">
								...
								<view class="cont_price color-red" style="margin: 0;">
									<text>USDT</text>
									{{ item.price || '' }}
								</view>
							</view>
...
export default {
	...
	methods: {
		...
		goDetails(id, status) {
			// uni.navigateTo({
			// 	url: '../../market/goods-def/goods-def-one?id=' + id
			// });
			// if (status == 0) {
			// 	uni.navigateTo({
