<template>
	<view class="page" :class="getThemeClass">
		...
		<view class="featured card" :class="getThemeClass" >
			<view class="featured_item card-bg" v-for="(item, index) in listData" :key="index" @click="goDetails(item.id)">
				<view class="plan">
					<image :src="baseUrl+item.products_cover || ''" mode="aspectFit"></image>
					<view class="soldout" v-if="item.total_issue==item.yiduihuan">
						已兑完
					</view>
				</view>
				<view class="featured_content">
					
					<view class="cont_title ">{{ item.products_title || '' }}</view>
					...
					<view class="flex justify-between align-center margin-top-20">
						...
						<view class="u-font-22 color-a">剩余：{{ item.total_issue-item.yiduihuan }}件</view>
					</view>
				</view>
			</view>
		</view>
	...
	methods: {
		...
		loadData(reset, initialize){
			...
			this.$u.api.jfshop({
				...
				var list = [{id:1,name:"代金券"},{id:2,name:"红包"},{id:3,name:"实物"}]
				if(initialize){
